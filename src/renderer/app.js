...
runBtn.addEventListener('click', async () => {
  ...
  if (!url) return shake(targetInput.closest('.url-input-wrap'));
  if (!/^https?:\/\/.+/.test(url)) {
  ...
  suites.forEach(s => {
    suiteOffset[s] = cumulativeTotal;
    cumulativeTotal += suiteTotals[s] || 0;
  });
  const grandTotal = cumulativeTotal;

  // Listen for progress events
  const cleanup = window.wafAPI.onProgress(({ suite, current, total, name }) => {
  ...
  try {
    const response = await window.wafAPI.runTests({ url, suites });
    cleanup();
...
function renderScoreSection() {
  ...
  // Populate live feed retrospectively
  liveFeedRows.innerHTML = '';
  for (const tests of Object.values(state.results)) {
    for (const t of tests) {
      addFeedRow(t.name, t.blocked);
    }
  }
}

function addFeedRow(name, blocked) {
...
function renderResultsTable() {
  ...
  const suiteLabels = {
    owasp: 'OWASP Top 10', ratelimit: 'Rate Limiting',
    bot: 'Bot Detection', bypass: 'Bypass Attempts'
  };

  let allTests = [];
  for (const [suite, tests] of Object.entries(state.results)) {
    tests.forEach(t => allTests.push({ ...t, suite }));
  }
  ...
  const rows = filtered.map(t => {
    ...
    const statusColor = t.status === 200 ? 'var(--muted)' : t.status >= 400 ? 'var(--green)' : 'var(--text)';
    return `<tr>
      ...
      <td class="td-result ${cls}">${label}</td>
  ...
  resultsTbl.innerHTML = `
    <table class="results-table">
      <thead>
        <tr>
          <th>Test</th><th>Payload</th><th>Status</th>
          <th>Latency</th><th>Result</th><th>Reason</th>
        </tr>
      </thead>
      <tbody>${rows}</tbody>
    </table>
...
async function doExportJson() {
  if (!state.results) {
    showExportStatus('No results yet — run tests first.', true);
    return;
  }
  try {
    ...
    showExportStatus(`Export error: ${err.message}`, true);
  }
}
...
function showExportStatus(msg, isError = false) {
  ...
  // Also update the in-page status on the Export view if visible
